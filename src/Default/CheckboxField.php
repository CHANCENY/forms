...
class CheckboxField extends FieldBase
{
    ...
    public function getBuildField(bool $wrapper = true): string
    {
        ...
        foreach ($this->field['checkboxes'] ?? [] as $radio) {

            if (!empty($this->field['limit']) && $this->field['limit'] > 1) {
                $id = "field-edit-".str_replace(' ','-',$radio);
                $class = $id . '-option';
                if (in_array($radio, $values)) {
                    $radios[] = "<div class='checkbox-option-field'><input type='checkbox' name='{$this->getName()}[]' id='{$id}' value='{$radio}' class='{$class}' checked/><label>{$radio}</label></div>";
                }
                else {
                    $radios[] = "<div class='checkbox-option-field'><input type='checkbox' name='{$this->getName()}[]' id='{$id}' value='{$radio}' class='{$class}'/><label>{$radio}</label></div>";
                }
            }
            else {
                $id = "field-edit-".str_replace(' ','-',$radio);
